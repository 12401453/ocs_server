...
int main () {
    ...
    sqlite3* DB;
    if(!sqlite3_open("orv_corpus_untagged.db", &DB)) {

        sqlite3_stmt* statement;

        std::ifstream chu_words_file("orv_words_full_with_titles_untagged.csv");
        std::ifstream lemma_spreadsheet("lemmas_with_text_occurence_gdrive.csv");
        std::ifstream chu_lemmas_file("orv_lemmas.csv");
        std::ifstream chu_subtitles_file("orv_subtitles.csv");
        ...
        while(std::getline(chu_words_file, line)) {
            ...
            autoreconstructed_lcs = autoreconstructed_array[0]; */ //AUTORECON
            autoreconstructed_lcs = "";
