...
int main () {
    ...
    sqlite3* DB;
    if(!sqlite3_open("chu_corpus_untagged_including_autorecon_autotagged.db", &DB)) {

        sqlite3_stmt* statement;

        std::ifstream chu_words_file("chu_words_full_with_titles_untagged_zogr_autolemma_and_tags_added.csv");
        ...
        while(std::getline(chu_words_file, line)) {
            ...
            if(lemma_id > 0 && torot_pos != "") sqlite3_bind_int(statement, 4, lemma_id);
