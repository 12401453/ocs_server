...
const convertToGlag = () => {
  ...
  document.querySelectorAll(".tooltip").forEach(tt => {
    ...
    tt.childNodes.forEach(child_node => child_node.textContent = cyrToGlag(child_node.textContent));
  });
  ttPosition();
};
const convertToCyr = () => {
...
const lcsSearch = (query, regex=false) => {
  ...
  if(app_state.search_type == 1){
    ...
    query = cleanWord(query, archiphonemeLCSMap); //converts ja/ša etc. to jǢ/šǢ
  }
...
const stealFromGorazdTOROT = (event) => {
  ...
  lookupGorazd(lemma_id, "", event.target);
};

const lookupGorazd = (lemma_id, query, event_target=null) => {
  ...
  const httpRequest = (method, url) => {
    ...
    xhttp.onreadystatechange = () => { 
        if (xhttp.readyState == 4) {
          ...
          displayDictBox(error_msg, event_target);
        }
    }; 
    xhttp.send(send_data);
  };
...
const displayDictBox = (error_msg, event_target=null) => {
  ...
  if(error_msg == "") {
    ...
    if(event_target) {
      ...
      if(conj_type != undefined && conj_type != "non_infl") {
        const dict_conj_table_btn_html_str = `<div id="dict_conj_table_btn" data-lemma_id="${lemma_id}" data-morph_tag="${morph_tag}" data-inflexion="${conj_type}" data-pv3="${pv2_3_exists}" title="Show inflection table for this lemma"><svg id="grid_symbol" width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
